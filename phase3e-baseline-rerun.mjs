// Phase 3E — Controlled Baseline Rerun: score floor 60, all regimes (incl. ranging)
// 36 runs: 6 windows × 3 symbols × 2 timeframes
// Same Playwright script approach as CondF/G — ensures apples-to-apples comparison
...
const OUT_FILE = path.join(__dirname, 'training-exports', 'phase3e-baseline-rerun-results.json');
...
const CONDITION = 'baseline-rerun';
...
async function runSim(page, run) {
  ...
  // Explicitly set score floor to 60 via 3rd range slider
  // Order: riskPerTrade(0), minConfidence(1), scoreFloor(2), maxOpenTime(3)
  ...
  // Do NOT click any regime pills — leave all regimes selected (incl. Ranging)
  // This is the controlled baseline: floor=60, all regimes

  // Click Run Simulation
  await page.getByRole('button', { name: '▶ Run Simulation' }).click();

  // Wait for result (up to 8 minutes — 15m data can take 3-4 min)
...
async function main() {
  ...
  console.log(`Phase 3E Controlled Baseline Rerun — Score Floor: ${SCORE_FLOOR}, All Regimes`);
  ...
  console.log(`\n✅ Phase 3E Baseline Rerun complete. ${results.length}/${RUNS.length} runs saved to ${OUT_FILE}`);
}
